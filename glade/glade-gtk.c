          ...
          const char *longname)
{
  ...
  for (tmp = info->children; tmp; tmp = tmp->next) {
    ...
    if (GTK_IS_MISC(child)) {
      gtk_misc_get_alignment(GTK_MISC(child),&xalign,NULL);
          ...
          GladeWidgetInfo *info, const char *longname)
{
  ...
  if (!tmp)
    return;
  entry = gtk_panda_combo_get_entry(GTK_PANDA_COMBO(w));
  for (tmp = cinfo->attributes; tmp; tmp = tmp->next) {
    GladeAttribute *attr = tmp->data;
    if (!strcmp(attr->name, "editable")) {
      gtk_editable_set_editable(GTK_EDITABLE(entry), attr->value[0] == 'T');
...
panda_text_new(GladeXML *xml, GladeWidgetInfo *info)
{
  ...
  gtk_editable_set_editable(GTK_EDITABLE(wid),editable);
  return wid;
}
...
{
  GtkWidget *ret = gtk_panda_progress_bar_new();
  GList *tmp;
  ...
  for (tmp = info->attributes; tmp; tmp = tmp->next) {
    ...
    if (!strcmp(attr->name, "value")) {
      gtk_panda_progress_bar_set_value(GTK_PANDA_PROGRESS_BAR(ret),
        g_strtod(attr->value, NULL));
    } else if (!strcmp(attr->name, "lower")) {
      gtk_panda_progress_bar_set_lower(GTK_PANDA_PROGRESS_BAR(ret),
        g_strtod(attr->value, NULL));
    } else if (!strcmp(attr->name, "upper")) {
      gtk_panda_progress_bar_set_upper(GTK_PANDA_PROGRESS_BAR(ret),
        g_strtod(attr->value, NULL));
    } else if (!strcmp(attr->name, "activity_mode")) {
      if (attr->value[0] == 'T') {
        // do nothing
      } else {
        gtk_progress_bar_pulse(GTK_PROGRESS_BAR(ret));
      }
      ...
      gtk_progress_bar_set_show_text(GTK_PROGRESS_BAR(ret),
        attr->value[0] == 'T');
      ...
      gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ret),
                   attr->value);
    }
  }
  return ret;
...
calendar_new (GladeXML *xml, GladeWidgetInfo *info)
{
  ...
  gtk_calendar_set_display_options(GTK_CALENDAR(wid), dopt);
  return wid;
}
